...
def solve_stokes(mesh, c, cname, ii):
    ...
    class Outer(SubDomain):
        def inside(self, x, on_boundary):
            return ((x[0]**2+x[1]**2+x[2]**2)**0.5 > 2)  and on_boundary
    surface = Surface()
    outer = Outer()
    ...
    with XDMFFile(MPI.comm_world, outdir+'velocity_c_'+str(int(c*100))+'_iter_'+str(ii)+'.xdmf') as file:
        file.write(u)
    with XDMFFile(MPI.comm_world, outdir+'pressure_c_'+str(int(c*100))+'_iter_'+str(ii)+'.xdmf') as file:
        file.write(p)
    ...
    if (mpiRank == 0):
        print([c, Fx, Fy, Fz, Tx, Ty, Tz])

    res = [c, Fx, Fy, Fz, Tx, Ty, Tz]
    return u,p,res

alpha_ref=0.02
if (mpiRank == 0):
    print('\nSOLVING CASE FOR RIGID ROTATION, FOR ALPHA=%f\n'%(alpha_ref))  
...
res = []
dres= []
u,p,resi=solve_stokes(mesh,0.,'0',0)
ncells = MPI.sum(comm,mesh.num_cells())
resi = [ncells] + resi
res.append(resi)
if (mpiRank == 0):
